  ...
  const {addPayments} = useAddPayment();
    return (
        <div className="w-[90%] mx-auto">
              ...
              <tbody className=" ">
            {
              addPayments?.map((payment , index) =>   <tr className={` ${index % 2 ==! 0 ?  'bg-gray-100 text-center' : 'bg-white text-center'} `} key={payment?._id}>
                <td>
                  {payment?.date}
                </td>
                <td>
                ...
                <td className="lg:text-xl py-5 flex items-start justify-center gap-5">
                                <svg xmlns="http://www.w3.org/2000/svg"  className="cursor-pointer bg-white shadow-custom-backend p-2 rounded-lg hover:bg-red-600 transition-all duration-300" width="40" height="40" viewBox="0 0 32 32" fill="none">
