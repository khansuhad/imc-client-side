...
import { fetchBatches, fetchClassBatches } from "../../../../Redux/Features/Batch/BatchSlice/BatchSlice";
import useAxiosPublic from "../../../../Hock/useAxiosPublic";


const ClassWiseBatches = () => {
  ...
  const updateProfileErrorToast = () => toast.error("Profile can't update");
   ...
   useEffect(() => {
     dispatch(fetchClassBatches({data : {searchTerm :id} , axiosInstance :useAxios })); // Fetch students whenever the searchTerm changes
   }, [dispatch, id,useAxios]);
   const refetchBatches = () => {
     dispatch(fetchClassBatches({data : {searchTerm :id} , axiosInstance :useAxios })); // Trigger refetch by dispatching fetchCourses
   };
